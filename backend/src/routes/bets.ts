...
router.post("/:bet_id/settle", verifyToken, async (req, res) => {
  ...
  if (!betData) {
    res.status(403).json({ error: 'Bet not exist or you are not the creator of this bet.' });
    return;
  }
...
router.post("/:bet_id/cancel", verifyToken, async (req, res) => {
  const bet_id = req.params.bet_id;
  const userId = res.locals.user.sub;
  ...
  // call supabase RPC
  const { data, error } = await supabase.rpc('cancel_bet', { p_bet_id: bet_id });
